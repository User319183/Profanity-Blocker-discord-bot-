...
with open(".txt/basefilterspan.txt") as file:
    basefilterspanish = file.read().split('\n')
...
class Spanish_Filter(commands.Cog):
        ...
        for word in basefilterspanish:
            if message.content.count(word) > 0:
                ...
                if len(message.content) > 1200:
                        ...
                        embed.add_field(name="Filter", value=f"Spanish", inline=False)
                        embed.timestamp = discord.utils.utcnow()
    ...
    async def on_ready(self):
        print('[READY] Cog "Spanish_Filter" has been loaded!')
        print(f'---------------------------------------')            
                    

    
def setup(bot):
    bot.add_cog(Spanish_Filter(bot))